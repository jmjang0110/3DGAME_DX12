...
void CTexture::UpdateShaderVariables()
{
	if (m_nRootParameters == m_nTextures)
	{
		for (int i = 0; i < m_nRootParameters; i++)
		{
			...
			{
				DX12_COMMAND_LIST->SetDescriptorHeaps(1, m_pd3dSrvDescriptorHeap.GetAddressOf());
				DX12_COMMAND_LIST->SetGraphicsRootDescriptorTable(m_pnRootParameterIndices[i]
...
void CTexture::CreateSrvDescriptorHeap()
{
	D3D12_DESCRIPTOR_HEAP_DESC d3dDescriptorHeapDesc{};
	d3dDescriptorHeapDesc.NumDescriptors = m_nTextures; // SRVs 
	d3dDescriptorHeapDesc.Type           = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
	d3dDescriptorHeapDesc.Flags          = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
	d3dDescriptorHeapDesc.NodeMask       = 0;

	DX12_DEVICE->GetDevice()->CreateDescriptorHeap(&d3dDescriptorHeapDesc
													, __uuidof(ID3D12DescriptorHeap)
													, (void**)m_pd3dSrvDescriptorHeap.GetAddressOf());
...
void CTexture::CreateShaderResourceViews(int nRootParameterStartIndex)
{

	for (int i = 0; i < m_nTextures; i++)
	{
		ID3D12Resource* pShaderResource = GetResource(i);
		D3D12_SHADER_RESOURCE_VIEW_DESC d3dShaderResourceViewDesc = GetShaderResourceViewDesc(i);
		DX12_DEVICE->GetDevice()->CreateShaderResourceView(pShaderResource, &d3dShaderResourceViewDesc, m_d3dSrvCPUDescriptorNextHandle);
		SetGpuDescriptorHandle(i, m_d3dSrvGPUDescriptorNextHandle);
	}
	int nRootParameters = GetRootParameters();
	for (int i = 0; i < nRootParameters; i++) SetRootParameterIndex(i, nRootParameterStartIndex + i);

}

void CTexture::UpdateShaderVariable(int nParameterIndex, int nTextureIndex)
{
	DX12_COMMAND_LIST->SetGraphicsRootDescriptorTable(m_pnRootParameterIndices[nParameterIndex]
...
void CTexture::LoadTextureFromDDSFile(const wchar_t* pszFileName, UINT nResourceType, UINT nIndex)
{
